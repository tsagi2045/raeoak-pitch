...
- **핵심 발견**:
  - "정통+소형+카운터" **교집합**이 비어 있음 (개별 요소는 경쟁자 존재)
  - 라이트의 핵심 전환: "축소"를 "밀착"으로 재정의
  - CI 간극 실존: 브로슈어(딥그린) vs 매장(우드+검정)
  - 3대 간극 위험: CI 불일치, 과소 아이덴티티, 포솔 자기잠식

## 2026-04-13 ingest | raw-12 수도권 소형 쌀국수 경쟁 조사

- **원본**: raw/raw-12-light-competitor-scan.md (웹 조사)
- **생성**: wiki/sources/raw-12-summary.md — 경쟁 매트릭스, 교집합 분석
- **갱신**:
  - wiki/output/라이옥-라이트-리브랜딩-진단.md — "비어있는 슬롯" → "비어있는 교집합"으로 수정, 미분당 바좌석·리틀하노이·꾸아 경쟁 반영
  - wiki/output/라이옥-라이트-포지셔닝.md — 경쟁 현황 반영, 차별점 경쟁 주의 컬럼 추가
  - wiki/index.md — Sources에 raw-12 추가
